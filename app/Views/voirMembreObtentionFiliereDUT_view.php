...
<script>
  $(document).ready(function() {
    ...
    function rechercherUsersObtentionFiliereDUT(query) {
      var idObtentionDUT=<?=$idObtentionDUT?>;
      $.ajax({
        method: "POST",
        url: "<?= base_url() ?>/public/user/rechercherUsersObtentionFiliereDUT",
        data: {
          'query': query,
          'id': idObtentionDUT
        ...
        success: function(response) {
          ...
          $.each(response.users, function(index, value) {
            if(value['photo']==0)
            {
              value['photo']="<?= base_url() ?>/public/importer/profiles/avatar.png"
            }
            $('.usersdata').append('\
            <div class="col-md-2 ">\
                  <div class="staff ">\
    ...
    $(document).on('click', '.pagination li', function() {
      var id = $(this).attr('id');
      getAllUsersLimitObtentionFiliereDUT(id);
      $('.usersdata').html('');
      $('.pagination').html('');
      $('.total_data').html('');
    });
  ...
  function getAllUsersLimitObtentionFiliereDUT(page) {
    var idObtentionDUT=<?=$idObtentionDUT?>;
    var idFiliereDUT=<?=$idFiliereDUT?>;
    $.ajax({
      method: "POST",
      url: "<?= base_url() ?>/public/user/getAllUsersLimitObtentionFiliereDUT",
      data: {
        'page': page,
        'id': idObtentionDUT,
        'idF': idFiliereDUT
